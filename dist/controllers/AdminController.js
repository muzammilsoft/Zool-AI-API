...
exports.toggleKeyStatus = exports.createKey = exports.updateAdminProfile = exports.showAdminDashboard = exports.logout = exports.login = exports.showLoginForm = exports.registerDeveloper = exports.showRegistrationForm = exports.initializeAdmin = void 0;
...
const registerDeveloper = async (req, res) => {
    ...
    // Save to DB
    await db.run('INSERT INTO developer_requests (first_name, last_name, email, company, reason) VALUES (?, ?, ?, ?, ?)', first_name, last_name, email, company || '', reason);
...
const login = async (req, res) => {
    ...
    if (match) {
        ...
        await db.run('UPDATE admins SET failed_attempts = ?, locked_until = ? WHERE id = ?', newAttempts, lockedUntil, admin.id);
...
const updateAdminProfile = async (req, res) => {
    ...
    if (password) {
        ...
        await db.run('UPDATE admins SET email = ?, password = ? WHERE id = ?', email, hashedPassword, adminId);
    }
    else {
        await db.run('UPDATE admins SET email = ? WHERE id = ?', email, adminId);
    }
...
const createKey = async (req, res) => {
    ...
    await db.run('INSERT INTO api_keys (key, first_name, last_name, email, type, limit_chat, limit_image, limit_audio, limit_video) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', key, first_name, last_name, email, type || 'standard', limit_chat || 1000, limit_image || 100, limit_audio || 100, limit_video || 10);
    res.redirect('/admin');
};
...
const toggleKeyStatus = async (req, res) => {
    ...
    await db.run('UPDATE api_keys SET status = ?, is_active = ? WHERE id = ?', status, status === 'active' ? 1 : 0, id);
    res.redirect('/admin');
};
